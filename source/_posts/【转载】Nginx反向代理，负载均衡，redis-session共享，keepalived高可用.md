---
title: 【转载】Nginx反向代理，负载均衡，redis session共享，keepalived高可用
date: 2020-03-23 16:47:55
categories: NGINX
tags:
- NGINX
---
...
![img](【转载】Nginx反向代理，负载均衡，redis-session共享，keepalived高可用/824587-20161210013029741-1131403264.png)**此时需要用到三台服务器，一台nginx服务器，两台正式部署项目的服务器：选择的是192.168.50.133主nginx和192.168.50.137,192.168.50.139两台tomcat服务器**
...
![img](【转载】Nginx反向代理，负载均衡，redis-session共享，keepalived高可用/824587-20161208231925601-1474979822.png)

编辑好后 service iptables restart 重新加载防火墙配置

![img](【转载】Nginx反向代理，负载均衡，redis-session共享，keepalived高可用/824587-20161208232235554-821786257.png)
...
![img](【转载】Nginx反向代理，负载均衡，redis-session共享，keepalived高可用/824587-20161210021128741-783517177.png)**将项目中web.xml中的访问顺序index.jsp上移到第一个访问**

 

**然后右键导出为war包，testproject.war，将该war包上传到两台服务器的tomcat的webapps中**

![img](【转载】Nginx反向代理，负载均衡，redis-session共享，keepalived高可用/824587-20161210021748835-63821191.png)
...
![img](【转载】Nginx反向代理，负载均衡，redis-session共享，keepalived高可用/824587-20161210022536257-563796782.png)

在两台tomcat的server.xml文件，Host标签中添加：<Context path="" docBase="testproject"/>，path标识访问路径，docBase为项目名，表示访问项目

![img](【转载】Nginx反向代理，负载均衡，redis-session共享，keepalived高可用/824587-20161210022853491-1479908862.png)

此时，重新启动tomcat，访问192.168.50.137:8080，192.168.50.139:8080,显示index.jsp内容：两台服务器访问显示如下

 ![img](【转载】Nginx反向代理，负载均衡，redis-session共享，keepalived高可用/824587-20161210024245991-1200437408.png)

![img](【转载】Nginx反向代理，负载均衡，redis-session共享，keepalived高可用/824587-20161210024301116-1896852396.png)
...
![img](【转载】Nginx反向代理，负载均衡，redis-session共享，keepalived高可用/824587-20161208233450960-1697266825.png)
...
![img](【转载】Nginx反向代理，负载均衡，redis-session共享，keepalived高可用/824587-20161208233822866-868810441.png)
...
![img](【转载】Nginx反向代理，负载均衡，redis-session共享，keepalived高可用/824587-20161208234258757-1246458568.png)
...
![img](【转载】Nginx反向代理，负载均衡，redis-session共享，keepalived高可用/824587-20161210040326319-1999583685.png)
...
```nginx
........location / {
        #加入判断，如果域名为test.baidubaidu.com，转发到192.168.50.133:80，然后再进行转发，注意，此处未进行测试，貌似是这么写的，$host为nginx变量，可以获取域名    
...
![img](【转载】Nginx反向代理，负载均衡，redis-session共享，keepalived高可用/824587-20161209013336616-1475474059.png)

 

本例中：

![img](【转载】Nginx反向代理，负载均衡，redis-session共享，keepalived高可用/824587-20161210032226226-287390560.png)
...
![img](【转载】Nginx反向代理，负载均衡，redis-session共享，keepalived高可用/824587-20161210035234491-2112324971.png)
...
![img](【转载】Nginx反向代理，负载均衡，redis-session共享，keepalived高可用/824587-20161210035545319-539003657.png)
...
![img](【转载】Nginx反向代理，负载均衡，redis-session共享，keepalived高可用/824587-20161209011942835-1106019887.png)
...
![img](【转载】Nginx反向代理，负载均衡，redis-session共享，keepalived高可用/824587-20161209012259272-825047638.png)
...
![img](【转载】Nginx反向代理，负载均衡，redis-session共享，keepalived高可用/824587-20161210041217944-704715113.png)

 修改content.xml

![img](【转载】Nginx反向代理，负载均衡，redis-session共享，keepalived高可用/824587-20161210041412757-1950717785.png)
...
**![img](【转载】Nginx反向代理，负载均衡，redis-session共享，keepalived高可用/824587-20161210053320116-1854584241.png)**
...
![img](【转载】Nginx反向代理，负载均衡，redis-session共享，keepalived高可用/824587-20161210133227866-1513518612.png)
...
![img](【转载】Nginx反向代理，负载均衡，redis-session共享，keepalived高可用/824587-20161210141242882-701187483.png)

 浏览器访问虚拟ip：192.168.50.88，效果如下

 ![img](【转载】Nginx反向代理，负载均衡，redis-session共享，keepalived高可用/824587-20161210141512835-1512216780.png)

 

 **备机：ip addr命令执行：可以看到，备机nginx没有绑定虚拟ip**

![img](【转载】Nginx反向代理，负载均衡，redis-session共享，keepalived高可用/824587-20161210141645413-2057533357.png)
...
![img](【转载】Nginx反向代理，负载均衡，redis-session共享，keepalived高可用/824587-20161210142401038-1734875767.png)

**备机情况：虚拟ip已经绑定到备机，此时主机虽然挂了，但是切换到备机上了（发现故障和切换的时间差最大也就是1秒），虚拟ip也绑到备机上了，访问虚拟ip，就会请求备机nginx然后转发到web服务器实现高可用。**

 ![img](【转载】Nginx反向代理，负载均衡，redis-session共享，keepalived高可用/824587-20161210143801194-1005494402.png)
...
**主机keepalived服务启动后，即吧主机维护好之后：可以看到，虚拟ip又自动绑到了主机上**
 ...
 ![img](【转载】Nginx反向代理，负载均衡，redis-session共享，keepalived高可用/824587-20161210144429507-2028529502.png)

 

 **备机情况，主机活了之后，备机转交管理权，虚拟ip切换到主机上，备机不绑定虚拟ip,貌似启动keepalived服务之后并不能马上切回，应该是起服务需要点时间吧，但是不影响，这段时间还是备机绑定虚拟IP的**

![img](【转载】Nginx反向代理，负载均衡，redis-session共享，keepalived高可用/824587-20161210144333101-695639547.png)
...
**主机挂了之后，主机nginx恢复时，一定要将nginx也启动，否则即使虚拟ip切换到了主机上，但是主机nginx没起那也是没法转发的。所以要把\**nginx启动要加在开机启动中。\****
...
将如下内容搞到该文件中:注意红色部分修改成你的路径即可，**nginxd**值是启动nginx的nginx路径，**nginx_config**值是nginx配置文件nginx.conf路径，**nginx_pid**值是nginx.pid所在路径，如果按照我方法安装的话，是在nginx安装目录的logs里边的
...
```
 ...
 ![img](【转载】Nginx反向代理，负载均衡，redis-session共享，keepalived高可用/824587-20161210151054194-750610516.png)
...
![img](【转载】Nginx反向代理，负载均衡，redis-session共享，keepalived高可用/824587-20161210164416866-1695444383.png)

然后执行脚本：

主机脚本检测nginx不在了，把keepalived停掉，从输出可以看到确实停止了，主机虚拟没有绑定虚拟ip

![img](【转载】Nginx反向代理，负载均衡，redis-session共享，keepalived高可用/824587-20161210164620554-1932400026.png)

备机：成功绑定虚拟ip

 ![img](【转载】Nginx反向代理，负载均衡，redis-session共享，keepalived高可用/824587-20161210164655538-1777482179.png)
...
![img](【转载】Nginx反向代理，负载均衡，redis-session共享，keepalived高可用/824587-20161210172645757-50735121.png)

 

 备机：绑定虚拟ip，切换成功。

 ![img](【转载】Nginx反向代理，负载均衡，redis-session共享，keepalived高可用/824587-20161210172826429-550094922.png)
...
下面是 nginx.conf 详细的配置文件介绍（**以下配置参数很多时候并不一定用的到，只是作为配置参数说明参考，可以看下面的通用版介绍**）
