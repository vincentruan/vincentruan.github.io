...
![OptionalParamWarning.png](OptionalParamWarning.png)
...
![OptionalGetWithoutIsPresent.png](OptionalGetWithoutIsPresent.png)
...
![LambdaExtractMethod.png](LambdaExtractMethod.png)

当然，这些甚至牵扯到了方法引用(Method References)。方法引用可能看着有点陌生，但是由于其能让代码达到更好的阅读性，还是有很大应用价值的。后面，我会讲到这个概念。

![LambdaCollapseToMethodRef.png](LambdaCollapseToMethodRef.png)

### 显式声明

在lambda表达式中是没有类型信息的，所以你会发现在参数中包含类型信息是非常有用的。

![LambdaExplicitParamTypes.png](LambdaExplicitParamTypes.png)

如你所见，这会变得很笨重。所以我更喜欢赋予参数有意义的名字。当然，无论你是否这么做，Intellij IDEA都会让你可以看到参数的类型信息。

![LambdaParamTypes.png](LambdaParamTypes.png)

甚至lambda表示的函数接口也能看到。

![LambdaFunctionalInterface.png](LambdaFunctionalInterface.png)
...
![LambdaExtractFunctionalParameter.png](LambdaExtractFunctionalParameter.png)
...
![LambdaFunctionalInterfaceNoMethod.png](LambdaFunctionalInterfaceNoMethod.png)
...
![LambdaFunctionalInterfaceNotInterface.png](LambdaFunctionalInterfaceNotInterface.png)
...
![StreamWrappingExample.png](StreamWrappingExample.png)

这样做也会让代码看起来很舒服。当然，如果这么做，会增加代码的行数。

你可以修改一下格式化设置使点号对齐。

![StreamWrapping.png](StreamWrapping.png)

### 使用方法引用(Method References)

你可能需要一会儿才能习惯这个奇怪的语法。但是，当我们能够正确地使用，它确实能够提高程序的可阅读性。考虑下面的代码：

![StreamSimpleFilter.png](StreamSimpleFilter.png)

对比一下使用新引入的Objects类的辅助方法(helper methods)：

![StreamMethodRefFilter.png](StreamMethodRefFilter.png)
...
![StreamRepalceForWithForEach.png](StreamRepalceForWithForEach.png)

使用Streams API相比起使用循环和if语句更加清晰明了。例如：

![StreamBefore.png](StreamBefore.png)

IDEA会建议重构为：

![StreamAfter.png](StreamAfter.png)
